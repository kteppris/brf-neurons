...
from brf_snn.modules import LICell, ALIFCell, ALIFCellBP, LICellBP
from brf_snn.functional import spike_deletion, quantize_tensor

# alpha parameter initialization
DEFAULT_ALIF_ADAPTIVE_TAU_M_MEAN = 20.0
DEFAULT_ALIF_ADAPTIVE_TAU_M_STD = 5.0


# rho parameter initialization
DEFAULT_ALIF_ADAPTIVE_TAU_ADP_MEAN = 20.0
DEFAULT_ALIF_ADAPTIVE_TAU_ADP_STD = 5.0
...
class ALIFRSNN_SD(SimpleALIFRNN):
    ...
    def forward(
        ...
        for t in range(sequence_length):
            ...
            hidden_z = spike_deletion(hidden_z, self.spike_del_p)
...
class ALIFRSNN_BP(SimpleALIFRNN):
    ...
    def forward(
        ...
        hidden_z = quantize_tensor(torch.zeros((batch_size, self.hidden_size)).to(x.device), self.bit_precision)
        hidden_u = torch.zeros_like(hidden_z)
        hidden_a = torch.zeros_like(hidden_z)
        out_u = quantize_tensor(torch.zeros((batch_size, self.output_size)).to(x.device), self.bit_precision)
