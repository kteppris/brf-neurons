...
from brf_snn.modules import BRFCell, LICell, RFCell, LICellBP
...
class RFRSNN_BP(SimpleVanillaRFRNN):
    ...
    def forward(
        ...
        x = quantize_tensor(x, self.bit_precision)
        hidden_z = quantize_tensor(torch.zeros((batch_size, self.hidden_size)).to(x.device), f=self.bit_precision)
        hidden_u = torch.zeros_like(hidden_z)
        hidden_v = torch.zeros_like(hidden_z)
